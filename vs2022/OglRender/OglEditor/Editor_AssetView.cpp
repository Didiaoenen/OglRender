	...
	Editor_AViewControllable(pTitle, pOpened, pWindowSettings)
{
	...
	mResource = static_cast<Render::Render_Model*>(nullptr);
...
void Editor::Editor_AssetView::_Render_Impl()
{
	...
	if (auto pval = std::get_if<Render::Render_Model*>(&mResource); pval && *pval)
	{
		mEditorRenderer.RenderModelAsset(**pval);
	}

	if (auto pval = std::get_if<Render::Render_Texture*>(&mResource); pval && *pval)
	{
		mEditorRenderer.RenderTextureAsset(**pval);
	}

	if (auto pval = std::get_if<Core::Core_Material*>(&mResource); pval && *pval)
	{
